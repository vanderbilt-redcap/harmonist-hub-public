...
$q = $module->query("SELECT record FROM ".getDataTable($pidsArray['METRICS'])." WHERE project_id = ? AND field_name=? order by value desc limit 1",[$pidsArray['METRICS'],"date"]);
$row = $q->fetch_assoc();
$metrics = $module->escape(\REDCap::getData($pidsArray['METRICS'], 'json-array', ['record_id' => $row['record']])[0]);

$array_sections = [0=>'requests',1=>'comments',2=>'users'];
$array_sections_title = [0=>'requests', 1=>'comments',2=>'users by region'];

$array_sections_data = [0=>'fileActivity',1=>'upRegion',2=>'downRegion'];
$array_sections_title_data = [0=>'file activity', 1=>'uploads by region',2=>'downloads by region'];

$sections_donuts = [0=>'requests',1=>'comments',2=>'users',3=>'fileActivity',4=>'upRegion',5=>'downRegion'];
$sections_donuts_title = [0=>'requests', 1=>'comments',2=>'users by region',3=>'file activity',4=>'uploads by region',5=>'downloads by region'];
...
$people = \REDCap::getData([
                                ...
                                'filterLogic' => "[last_requested_token_d] <> ''"
                            ]);
...
$requests_values = $module->escape([0 => $number_concepts,1=> $number_abstracts,2 => $number_manuscripts,3 => $number_fastTrack,4 => $number_poster ,5 => $number_other]);
$requests_labels = $module->escape([0 => "Concepts",1 => "Abstracts",2 => "Manuscripts",3 => "Fast Track",4 => "Poster", 5=>"Other"]);
$requests_colors = $module->escape([0 => "#337ab7",1 => "#00b386",2 => "#f0ad4e",3 => "#ff9966",4 => "#5bc0de",5 => "#777"]);

$comments_values = $module->escape([0 => $metrics['comments_pi'],1 => $metrics['comments_n']]);
$comments_labels = $module->escape([0 => "PI",1 => "non PI"]);
$comments_colors = $module->escape([0 => "#1ad1ff",1 => "#eb6e60"]);
...
$number_deletes = $module->escape(count(\REDCap::getData([
                               'project_id' => $pidsArray['DATAUPLOAD'],
                               'return_format' => 'json-array',
                               'filterLogic' => "[deleted_y] = '1' AND [deletion_type] = '2'"
                           ])));
$number_deletes_auto = $module->escape(count(\REDCap::getData([
                               'project_id' => $pidsArray['DATAUPLOAD'],
                               'return_format' => 'json-array',
...
foreach ($regions as $region){
    ...
    $RecordSetDU = \REDCap::getData([
                                    'project_id' => $pidsArray['DATAUPLOAD'],
                                    'return_format' => 'json-array',
                                    'filterLogic' => "[data_upload_region] = '".$region['record_id']."'"
                                ]);
    array_push($upRegion_values,count($RecordSetDU));
    $RecordSetDN = \REDCap::getData([
                                    'project_id' => $pidsArray['DATADOWNLOAD'],
                                    'return_format' => 'json-array',
                                    'filterLogic' => "[downloader_region] = '".$region['record_id']."'"
                                ]);
...
#TITLES AND COLORS
$fileActivity_labels = $module->escape([0 => "Uploads",1 => "Downloads",2 => "Manual Delete"]);
$fileActivity_colors = $module->escape([0 => "#5cb85c",1 => "#337ab7",2 => "#eb6e60"]);
...
$start_date_datacall_Y = date('Y',strtotime(arrayKeyExistsReturnValue($request_dataCall, [1, 'sop_created_dt'])));
$start_date_datacall_m = date('m',strtotime(arrayKeyExistsReturnValue($request_dataCall, [1, 'sop_created_dt'])));
$time_labels = [];
$dataCall_started_timeline = [];
$dataCall_ended_timeline = [];
$yearfound = false;
...
$unique_data_downloaders = [];
$dataCall_timeline_labels = [];
$dataReq_values = [];
if(is_array($request_dataCall) && !empty($request_dataCall)) {
    foreach ($request_dataCall as $dataCall) {
        if ($dataCall['sop_visibility'] == 1) {
            $dataReq_values[0] += 1;
        } else {
            if ($dataCall['sop_visibility'] == 2) {
                if ($dataCall['sop_status'] == '0') {
                    $dataReq_values[1] += 1;
                } else {
                    if ($dataCall['sop_status'] == '1') {
                        $dataReq_values[2] += 1;
                    }
                }
            }
        ...
        if ($dataCall['sop_closed_y'] == '1') {
            ...
            $RecordSetDU = \REDCap::getData([
                                                'project_id' => $pidsArray['DATAUPLOAD'],
                                                'return_format' => 'json-array',
                                                'filterLogic' => "[data_assoc_request] = '" . $dataCall['record_id'] . "'"
                                            ]);
        ...
        if ($dataCall['sop_created_dt'] != "" && $dataCall['sop_final_d'] != "") {
            ...
            foreach ($time_labels as $index => $time) {
                ...
                if ($start_found && (strtotime($time) <= strtotime($dataCall['sop_final_d']) || !array_key_exists(
                            'sop_final_d',
                            $dataCall
                        ) || $dataCall['sop_final_d'] == '')) {
                ...
                if ($end_found && (strtotime($time) <= strtotime($dataCall['sop_closed_d']) || !array_key_exists(
                            'sop_closed_d',
                            $dataCall
                        ) || $dataCall['sop_closed_d'] == '')) {
...
$people = \REDCap::getData([
                                    'project_id' => $pidsArray['PEOPLE'],
                                    'return_format' => 'json-array',
                                    'field' => ['first_ever_login_d','active_y','inactive_d','harmonist_regperm']
                                ]);
$array_communications = [];
$communications_years = [];
                ...
                options: {
                    ...
                    plugins: {
                        ...
                        datalabels: {
                            ...
                            anchor: 'end', // Positioning: End of the segment
                            align: 'end', // Alignment: End of the segment
                    ...
                    animation: {
                        onComplete: function () {
                            // Use `this.canvas` to get the chart canvas
                            document.querySelector('#down' + sections_donuts[section]).setAttribute('href', this.canvas.toDataURL());
                        }
                    }
                },
            ...
            options: {
                ...
                plugins: {
                    legend: {
                        ...
                        onClick: function(e, legendItem) {
                            ...
                            var ci = this.chart;
                            var alreadyHidden = ci.getDatasetMeta(index).hidden ?? false; // Check if the dataset is hidden
                    ...
                    tooltip: {
                        callbacks: {
                            title: function(tooltipItem) {
                                const index = tooltipItem[0].dataIndex; // Get the index of the tooltip item
                                const preDataCalls = tooltipItem[0].chart.data.datasets[0].data[index];
                                const activeDataCalls = tooltipItem[0].chart.data.datasets[1].data[index];
                                return `Pre-Data Call Requests: ${preDataCalls}\nActive Data Calls: ${activeDataCalls}`;
                            },
                            label: function(tooltipItem) {
                                const index = tooltipItem.dataIndex; // Get the index of the tooltip item
                                if (dataCall_timeline_labels[index] !== undefined) {
                                    const text = dataCall_timeline_labels[index];
                                    const titles = text.split("\n");
                                    return titles.filter(title => title !== ""); // Filter out empty lines
                                }
                            }
                        },
                        backgroundColor: '#FFF',
                ...
                animation: {
                    onComplete: function() {
                        ...
                        document.querySelector('#downdatacalltimeline')
            ...
            options: {
                ...
                plugins: {
                    legend: {
                        ...
                        onClick: (e, legendItem) => {
                            ...
                            const ci = e.chart;
                            const alreadyHidden = ci.getDatasetMeta(index).hidden ?? false;

                            ci.data.datasets.forEach((dataset, i) => {
                                const meta = ci.getDatasetMeta(i);
                                if (i !== index) {
                                    meta.hidden = !alreadyHidden ? (meta.hidden ?? !meta.hidden) : true;
                                } else {
                                    meta.hidden = null;
                                }
                            });
                ...
                animation: {
                    onComplete: (animation) => {
                        document.querySelector('#downcommunications').setAttribute('href', animation.chart.toBase64Image());
                    },
                },
            },
        };
...
<div class="container">
    <h6>General Hub Stats</h6>
    <p class="hub-title"><?=filter_tags($settings['hub_statistics_gen_text'])?></p>
    <p class="hub-title"><i class="fa fa-refresh" aria-hidden="true"></i> Last update on <span class="fw-bold"><?=$metrics['date']?></span></p>
</div>
    ...
    <?php foreach ($sections_donuts as $index=>$section) {
        if ($index <= 2) {
            ...
            <div class="canvas_title"><?= $section ?>
                <a href="#" download="<?= $sections_donuts_title[$index] . ".png" ?>" class="fa fa-download text-secondary ps-2"
                   id="<?= "down" . $section ?>"
                   name="<?= "down" . $section ?>"></a>
            </div>
        <?php }
...
<div class="container pb-4">
    <div class="container">
        <!-- First Row: Votes, Requests, Publications, and Abstracts -->
        <div class="row g-4">
            <div class="col-md-4">
                <div class="card h-100 mb-2">
                    <div class="card-header">
                        <h6 class="card-title">
                            Votes <span class="badge bg-primary float-end"><?=$metrics['votes']?></span>
                        </h6>
                    </div>
                    <div class="stat-table-outer" aria-expanded="true">
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of times "vote later" was used</span>
                            <span class="stat-table-value"><?=$metrics['vote_later'];?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of revisions uploaded</span>
                            <span class="stat-table-value"><?=$metrics['revisions'];?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of votes completed before due date</span>
                            <span class="stat-table-value"><?=$metrics['votes_c'];?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-percent text-primary"></i> of votes completed before due date</span>
                            <span class="stat-table-value"><?=$metrics['votes_c_percentage'];?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of votes completed after due date</span>
                            <span class="stat-table-value"><?=$metrics['votes_late'];?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-percent text-primary"></i> of votes completed after due date</span>
            ...
            <div class="col-md-4">
                <div class="card h-100 mb-2">
                    <div class="card-header">
                        <h6 class="card-title">
                            Requests <span class="badge bg-primary float-end"><?=$metrics['requests']?></span>
                        </h6>
                    </div>
                    <div class="stat-table-outer" aria-expanded="true">
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> requests currently approved by admin</span>
                            <span class="stat-table-value"><?=$metrics['requests_a'];?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of requests currently rejected by admin</span>
                            <span class="stat-table-value"><?=$metrics['requests_r'];?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of requests currently deactivated by admin</span>
                            <span class="stat-table-value"><?=$metrics['requests_d'];?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of requests completed (overall)</span>
            ...
            <div class="col-md-4">
                <div class="card h-40 mb-3">
                    <div class="card-header">
                        <h6 class="card-title">
                            Publications <span class="badge bg-primary float-end"><?=$metrics['publications']?></span>
                        </h6>
                    </div>
                    <div class="stat-table-outer" aria-expanded="true">
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of publications so far in current year</span>
                ...
                <div class="card h-40 mb-2">
                    <div class="card-header">
                        <h6 class="card-title">
                            Abstracts <span class="badge bg-primary float-end"><?=$metrics['abstracts']?></span>
                        </h6>
                    </div>
                    <div class="stat-table-outer" aria-expanded="true">
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of abstracts so far in current year</span>
        ...
        <!-- Second Row: Comments, Concepts, and Users -->
        <div class="row g-4 mt-1">
            <div class="col-md-4">
                <div class="card h-100">
                    <div class="card-header">
                        <h6 class="card-title">
                            Comments <span class="badge bg-primary float-end"><?=$metrics['comments']?></span>
                        </h6>
                    </div>
                    <div class="stat-table-outer" aria-expanded="true">
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of comments by PI-level users</span>
                            <span class="stat-table-value"><?=$metrics['comments_pi'];?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of comments by non-PI-level users</span>
            ...
            <div class="col-md-4">
                <div class="card h-100">
                    <div class="card-header">
                        <h6 class="card-title">
                            Concepts <span class="badge bg-primary float-end"><?=$metrics['concepts']?></span>
                        </h6>
                    </div>
                    <div class="stat-table-outer" aria-expanded="true">
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of active concepts</span>
                            <span class="stat-table-value"><?=$metrics['concepts_a']?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of completed concepts</span>
                            <span class="stat-table-value"><?=$metrics['concepts_c']?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of discontinued concepts</span>
            ...
            <div class="col-md-4">
                <div class="card h-100">
                    <div class="card-header">
                        <h6 class="card-title">
                            Users <span class="badge bg-primary float-end"><?=$metrics['users']?></span>
                        </h6>
                    </div>
                    <div class="stat-table-outer" aria-expanded="true">
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of PI-level users</span>
                            <span class="stat-table-value"><?=$metrics['users_pi'];?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of users with Access Link in last 30 days</span>
                            <span class="stat-table-value"><?=$metrics['users_access'];?></span>
                        </div>
                        <div>
                            <span><i class="fa fa-hashtag text-primary"></i> of Harmonist Admins</span>
    ...
    <div class="row">
        <div class="col-sm-4">
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title">
                        Data Requests <span class="badge bg-primary float-end"><?=$dataCall_total?></span>
                    </h6>
                </div>
                <div class="stat-table-outer" aria-expanded="true">
                    <div>
                        <span><i class="fa fa-hashtag text-primary"></i> private</span>
                        <span class="stat-table-value"><?=$dataReq_values['0'];?></span>
                    </div>
                    <div>
                        <span><i class="fa fa-hashtag text-primary"></i> public draft</span>
                        <span class="stat-table-value"><?=$dataReq_values['1'];?></span>
                    </div>
                    <div>
                        <span><i class="fa fa-hashtag text-primary"></i> public final</span>
                        <span class="stat-table-value"><?=$dataReq_values['2'];?></span>
                    </div>
                    <div>
                        <span><i class="fa fa-hashtag text-primary"></i> data call closed</span>
                        <span class="stat-table-value"><?=$dataCall_closed;?></span>
                    </div>
                    <div>
                        <span>avg <i class="fa fa-hashtag text-primary"></i> uploads per request</span>
        ...
        <div class="col-sm-4">
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title">
                        File Activity <span class="badge bg-primary float-end"><?=$fileActivity_total?></span>
                    </h6>
                </div>
                <div class="stat-table-outer" aria-expanded="true">
                    <div>
                        <span><i class="fa fa-hashtag text-primary"></i> uploads</span>
                        <span class="stat-table-value"><?=$fileActivity_values['0'];?></span>
                    </div>
                    <div>
                        <span><i class="fa fa-hashtag text-primary"></i> downloads</span>
                        <span class="stat-table-value"><?=$fileActivity_values['1'];?></span>
                    </div>
                    <div>
                        <span><i class="fa fa-hashtag text-primary"></i> manual deletes</span>
                        <span class="stat-table-value"><?=$fileActivity_values['2'];?></span>
                    </div>
                    <div>
                        <span><i class="fa fa-hashtag text-primary"></i> automatic deletes</span>
        ...
        <div class="col-sm-4">
            <div class="card">
                ...
                <div class="stat-table-outer" aria-expanded="true">
                    <div>
                        <span><i class="fa fa-hashtag text-primary"></i> data downloaders</span>
                        <span class="stat-table-value"><?=$data_downloaders;?></span>
                    </div>
                    <div>
                        <span><i class="fa fa-hashtag text-primary"></i> datasets uploaded by due date</span>
                        <span class="stat-table-value"><?=$datasets_up_due;?></span>
                    </div>
                    <div>
                        <span><i class="fa fa-percent text-primary"></i> datasets uploaded by due date</span>
                        <span class="stat-table-value"><?=round((($datasets_up_due/$dataCall_total)*100),2);?></span>
                    </div>
                    <div>
                        <span><i class="fa fa-hashtag text-primary"></i> datasets uploaded after due date</span>
                        <span class="stat-table-value"><?=$datasets_up_afterdue;?></span>
                    </div>
                    <div>
                        <span><i class="fa fa-percent text-primary"></i> datasets uploaded after due date</span>
...
<!--    </h4>-->
<!--    <p class="hub-title">--><?php //=filter_tags($settings['hub_stats_datacall_time'])?><!--</p>-->
<!--</div>-->
<!--<div class="container">-->
...
<div class="container pt-5">
    <h4>
        ...
        <a href="#" download="communications.png" class="fa fa-download text-secondary ps-2" id="downcommunications" name="downcommunications"></a>
    </h4>

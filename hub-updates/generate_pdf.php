...
$criticalVariables = HubUpdates::organizeByConstantAndCriticality(ArrayFunctions::csvToArray($module->getUrl('csv/HubUpdatesVariableCategories.csv')));
...
.critical{background-color:#cb410b;}
.medium{background-color:#E3CE6B;}
.low{background-color:#87C973;}
tr.rowSelected td{
...
<html lang="en">
    ...
    foreach ($allUpdates as $constant => $project_data) {
        ...
        if(arrayKeyExistsReturnValue($criticalVariables,[$constant,HubUpdates::VARIABLE_CRITICALITY_TOTAL,HubUpdates::VARIABLE_CRITICALITY_CRITICAL]) !== null){
            $criticalVariablesTotal = $criticalVariables[$constant][HubUpdates::VARIABLE_CRITICALITY_TOTAL][HubUpdates::VARIABLE_CRITICALITY_CRITICAL];
        }
        ...
        if($criticalVariablesTotal > 0){
            $html_pdf .= '<div><span class="label removed labeltext">&nbsp;!&nbsp;</span><span class="hub-update-last-updated-past-date"> There are ' . $criticalVariablesTotal . ' critical variables that need attention.</span></div></br>';
        }

        $html_pdf .= '<table class="table sortable-theme-bootstrap" data-sortable style="width: 100%;">
                <tr style="width: 100%;">
                ...
                <tr style="width: 100%;">
                    <td colspan="5" style="text-align: left !important;">
                        ...
                        <span class="label critical">&nbsp;!&nbsp;</span><span>&nbsp;Critical</span>
                        &nbsp;&nbsp;&nbsp;&nbsp;<span class="label medium">=</span><span>&nbsp;Medium</span>
                        &nbsp;&nbsp;&nbsp;&nbsp;<span class="label low">▼</span><span>&nbsp;Low</span>
                    </td>
                </tr>
            ...
            if ($instrument != "TOTAL") {
                ...
                foreach ($instrumentData as $status => $typeData) {
                    foreach ($typeData as $variable => $data) {
                        $html_pdf .= '<tr>
                                    <td>' . HubUpdates::getCriticalityIcon($criticalVariables, $constant, $variable, "pdf") . HubUpdates::getIcon($status, "pdf") . '</td>

...
$resolved_list = HubUpdates::getResolvedList($module,'resolved', $pidsArray);
...
?>
    ...
    <br><br>
    <?php
    // Display alerts that were dismissed
    HubUpdates::displayAlerts($module, $pidsArray, $settings, true);
    ...
    <div class="container-fluid p-y-1" style="margin-top:40px">
        ...
        foreach ($resolved_list as $constant => $variablesData) {
            $criticalityData = HubUpdates::organizeCriticalityByIgnoreList($module, $pidsArray[$constant], $constant, $variablesData);
            ...
            foreach ($variablesData as $index => $variable) {
                ...
                $icon = "";
                if(arrayKeyExistsReturnValue($criticalityData,[$variable['field_name']]) !== null && $criticalityData[$variable['field_name']] == HubUpdates::VARIABLE_CRITICALITY_CRITICAL) {
                    $icon = HubUpdates::getCriticalityIcon($criticalityData, $variable['field_name']);
                }

                $printProject = $icon."<span style='vertical-align: -3px;'>#".$project_id." - ".$title." => <strong>".$variable['field_name']."</strong> (<em>".$variable['field_type']."</em>)</span>";
                ...
                $hasVariableBeenUpdated = HubUpdates::hasVariableBeenUpdated($allUpdates['data'][$constant], $variable['field_name']);
                $varFound = false;
                ?>
            <tr>
                                    ...
                                    <td>
                                        ...
                                        if(is_array($hub_updates_resolved_list_last_updated)){
                                            ...
                                            <span class="hub-update-last-updated">
                                                ...
                                                if($hasVariableBeenUpdated && isset($hub_updates_resolved_list_last_updated[$constant][$variable['field_name']]['date'])) {
                        ...
                        <table class="table sortable-theme-bootstrap" data-sortable>
                            ...
                            foreach ($allUpdates['data'][$constant] as $instrument => $instrumentData){
                                if($instrument != "TOTAL"){
                                    ...
                                    foreach ($instrumentData as $status => $typeData){
                                        foreach ($typeData as $variableChanges => $data){
                                            if($variableChanges == $variable['field_name']){
                                                ...
                                                ?>
                                                <tr>
                                                    <td><?= HubUpdates::getCriticalityIcon($criticalityData, $variable['field_name']) ?><?=HubUpdates::getIcon($status)?></td>
                                    ...
                                    if(!$varFound && !$hasVariableBeenUpdated){
                                        //Show some data for deleted variables
                                        ?>
                                        <tr>
                                            <td><?= HubUpdates::getCriticalityIcon($criticalityData, $variable['field_name']) ?><?=HubUpdates::getIcon($variable['field_status'])?></td>
                                            <td><?=$variable['field_name']?></td>
                                            <td>
                                            </td>
                                            <td class="col-sm-4">

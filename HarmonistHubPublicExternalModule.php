...
class HarmonistHubPublicExternalModule extends AbstractExternalModule
{
    ...
    public function redcap_module_link_check_display($project_id, $link)
    {
        ...
        if ($link['name'] == "Harmonist Hub") {
            ...
            if (count($dd_array) == 1 && $isAdmin && !array_key_exists(
                ...
                // Assign url and remove pid as REDCap adds it later
                $link['url'] = preg_replace('/&pid=\d+/', '',  $this->getUrl("installProjects.php"));
            }
            ...
            if($project_id == $hub_mapper){
                // Assign url and remove pid as REDCap adds it later
                $link['url'] = preg_replace('/&pid=\d+/', '',  $this->getUrl("hub-user-management/redirectToPeopleDataDownloadsUserManagement.php"));
            }
        } else {
            #User has no permissions to see Last Updates, do not show link
            if (!$this->getUser()->hasDesignRights($project_id)) {
    ...
    function addProjectToList($project_id, $eventId, $record, $fieldName, $value)
    {
        try {
            // Execute the query
            $result = $this->query(
                "INSERT INTO " . getDataTable($project_id) . " (project_id, event_id, record, field_name, value) VALUES (?, ?, ?, ?, ?)",
                [$project_id, $eventId, $record, $fieldName, $value]
            );
        } catch (\Exception $e) {
            // Log the error
            error_log("Error in addProjectToList: " . $e->getMessage());
        }
    }
    ...
    function redcap_save_record($project_id, $record, $instrument, $event_id)
    {
        ...
        try {
            ...
            $sessionTokenGroup = $_SESSION[SecurityHandler::SESSION_TOKEN_STRING] ?? null;
            $tokenSessionName = $this->getSecurityHandler()->getTokenSessionName();
            $token = (is_array($sessionTokenGroup) && isset($sessionTokenGroup[$tokenSessionName]))
                ? $sessionTokenGroup[$tokenSessionName]
                : null;

            if ($token !== null && $token !== '') {
                $this->getSecurityHandler()->setSessionDataFromToken($token);
            }
    ...
    function redcap_survey_acknowledgement_page($project_id, $record, $instrument, $event_id)
    {
        ...
        $this->getSecurityHandler()->setSessionDataFromToken($this->getSecurityHandler()->getTokenFromSurveyReset());
    ...
    function cronMethod($cronAttributes)
    {
        ...
        if($cronAttributes['cron_name'] == 'cron_publications' && $this->isTodayTheWeekend()){
            // On the weekend send at a different time

            $hourStart = 6; // Starting hour (6 AM)
            $hourEnd = 12; // Ending hour (12 PM)

            // Check if the current hour is outside the range (6 AM to noon)
            if (date('G') < $hourStart || date('G') >= $hourEnd) {
                // Only perform actions between 6 AM and noon
                return;
            }

            if($this->hasTheCronRunRecently($lastRunSettingName, $hourStart)){
                // We're already run recently
                return;
            }
        }else if ($cronAttributes['cron_name'] != 'cron_data_upload_notification' && $cronAttributes['cron_name'] != 'cron_req_finalized_notification') {
            ...
            if($this->hasTheCronRunRecently($lastRunSettingName, $hourRange)){
                // We're already run recently
                return;
            }

            if ($cronAttributes['cron_name'] == 'cron_monthly_digest' || $cronAttributes['cron_name'] == 'cron_hub_updates_data_notification') {
                if (!$this->isTodayMonday()) {
                    //It's not Monday
                    return;
                }
                if($cronAttributes['cron_name'] == 'cron_monthly_digest' && !$this->isTodayFirstMondayOfTheMonth()) {
                        ...
                        if (!empty($settings)) {
                            try {
                                ...
                                if ($cronAttributes['cron_name'] == 'cron_metrics' && $settings['deactivate_metrics_cron___1'] !== "1") {
                                    ...
                                    //Check if we need to generate the Publications PDF
                                    if($this->getProjectSetting('hub-update-publications', $project_id)){
                                        //User clicked on Admin->Update Publications
                                        //Update Publications
                                        $isAdmin = true;
                                        include("crontasks/cron_publications.php");
                                        //Set last update date and check
                                        $this->setProjectSetting('hub-publications-updated', true, $project_id);
                                        $this->setProjectSetting('hub-publications-updated-date', date("Y-m-d H:i:s"), $project_id);
                                        $this->setProjectSetting('hub-update-publications', false, $project_id);
                                    }
                                ...
                                } elseif ($cronAttributes['cron_name'] == 'cron_hub_updates_data_notification') {
                                    //Every Monday
                                    include("crontasks/cron_hub_updates_data_notification.php");

                                    //Admin Notifications
                                    $adminEmails = $this->getProjectSetting('admin-notifications', $project_id);
                                    if(!empty($adminEmails)) {
                                        $adminFrequency = $this->getProjectSetting('admin-frequency', $project_id) ?? 7;
                                        $adminFrequencyLastSent = $this->getProjectSetting('admin-frequency-last-sent', $project_id);

                                        $shouldSendNotification = false;

                                        if (!empty($adminFrequencyLastSent)) {
                                            $daysSinceLastSent = (time() - strtotime($adminFrequencyLastSent)) / (60 * 60 * 24); // Calculate days difference
                                            $shouldSendNotification = $daysSinceLastSent >= $adminFrequency;
                                        } else {
                                            $shouldSendNotification = true; // No last sent date, send notification
                                        }

                                        if ($shouldSendNotification) {
                                            include("crontasks/cron_hub_updates_admin_required_notification.php");
                                        }
                                    }
                                }
                            } catch (Throwable $e) {
        ...
        if($cronAttributes['cron_name'] == 'cron_publications' && $this->isTodayTheWeekend()
            ||
            $cronAttributes['cron_name'] != 'cron_data_upload_notification' && $cronAttributes['cron_name'] != 'cron_req_finalized_notification'
        ) {
...
//        $this->getTwig()->addFunction($function);
    ...
    public function checkAuthorization($module) {
        ...
        if ($module->getSecurityHandler()->isAuthorizedPage()) {
            ...
            $tokenSession = $_SESSION[SecurityHandler::SESSION_TOKEN_STRING][$module->getSecurityHandler()->getTokenSessionName()] ?? null;
            if ($settings['deactivate_datahub___1'] != "1" && !empty($tokenSession)) {
                $is_authorized_and_has_rights = true;
            }
        }
        return $is_authorized_and_has_rights;
    }
    ...
    private function isTodayMonday(){
    ...
    private function isTodayFirstMondayOfTheMonth(){
    ...
    private function isTodayTheWeekend(){
        if (date('w', strtotime(date('Y-m-d'))) == '6' || date('w', strtotime(date('Y-m-d'))) == '0') {
            return true;
        }
        return false;
    }

    private function hasTheCronRunRecently($lastRunSettingName, $hourRange){

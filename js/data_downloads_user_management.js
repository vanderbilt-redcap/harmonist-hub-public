    ...
    $('#add_user').click(function () {
        ...
        if(checked_values.length != 0) {
            ...
            checked_values.forEach((user_id) => {
                ...
                error_list += "</ul>";
                users_info += '<li class="error-list-element"><span>' + $('#' + user_id).attr("user-data")+"</span>";
                ...
                users_info += '<div><span class="user">REDCap user: </span>';
                let userNameMissing = $('#error-list-'+user_id).attr('username-missing');
                if(userNameMissing) {
                    users_info += '<input type="text"class="form-control" placeholder="Select REDCap user" onkeyup="checkUserName(' + user_id + ')" name="new_user" id="user-name-' + user_id + '"></li>' +

...
foreach ($years as $year){
    ...
    $page_content = "";
    foreach ($concepts as $concept) {
        if (!isset($concept['output_year']) || !is_array($concept['output_year'])) {
            continue;
        }
        foreach ($concept['output_year'] as $index => $value) {
            //only manuscripts
            if ($value == $year
                && is_array($concept['output_type'] ?? null)
                && ($concept['output_type'][$index] ?? '') == '1') {
                $number++;
                $page_content .= '<div style="padding: 10px;"><table><tr><td style="vertical-align: text-top;width:30px">'.$number.'. </td><td>'
                    .htmlentities($concept['output_authors'][$index] ?? '').". "
                    .htmlentities($concept['output_title'][$index] ?? '').". "
                    .($concept['output_year'][$index] ?? '')
                    .", ".htmlentities($concept['output_venue'][$index] ?? '').". "
                    .htmlentities($concept['output_citation'][$index] ?? '')
                    .". <a href='https://www.ncbi.nlm.nih.gov/pmc/articles/".($concept['output_pmcid'][$index] ?? '')."' target='_blank'>"
                    .htmlentities($concept['output_pmcid'][$index] ?? '')."<i class='fa fa-fw fa-external-link' aria-hidden='true'></i></a>. Associated with IeDEA concept ". $concept['concept_id'].'</td></tr></table></div>';
            }
        }
    }

...
$request_id = htmlentities($_REQUEST['record'],ENT_QUOTES);
...
foreach ($region_vote_values as $votes_info){
    ...
    if($region !== "" && $vote !== "") {
        if (arrayKeyExistsReturnValue($request, ['region_vote_status',$region]) != $vote && $vote != "") {
            $date = new \DateTime();
...
if($all_votes_completed && !empty($request_id)){

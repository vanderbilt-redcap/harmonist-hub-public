...
$isUserConcepts = arrayKeyExists($_REQUEST,'user') ?? false;
$concepts_table = "";
$concepts = $module->getConceptModel()->fetchAllConcepts();
if (!empty($concepts)) {
    $concepts_table .= '<table class="table table-hover sortable-theme-bootstrap concepts-table" data-sortable id="sortable_table">'.
        '<colgroup>'.
        '<col>'.
        '<col>'.
        '<col>'.
        ...
        '<th class="sorted_class" scope="col" data-sorted="true" data-sorted-direction="descending">ID</th>'.
        '<th class="sorted_class" scope="col" data-sorted="true">Working Group</th>'.
        '<th class="sorted_class" scope="col" data-sorted="true">Tags</th>'.
        '<th class="sorted_class" scope="col" data-sorted="true">Active</th>'.
        '<th class="sorted_class" scope="col" data-sorted="true">Working Group 2</th>'.
        '<th class="sorted_class" scope="col">Contact</th>'.
        '<th class="sorted_class" scope="col">Concept Title</th>'.
        '<th class="sorting_disabled" scope="col" data-sortable="false">Concept</th>'.
        '</tr></thead><tbody>';

    foreach ($concepts as $concept) {
        if(!$isUserConcepts || ($isUserConcepts && ($concept->getContactLink() == $currentUser['record_id'] || $concept->getContact2Link() == $currentUser['record_id']))) {
            ...
            $concepts_table .= '<tr wg_id="' . $concept->getWgLink() . '">' .
                    ...
                    '<td>' . htmlspecialchars($concept->getWgLink(), ENT_QUOTES) . '</td>' .
                    '<td>' . htmlspecialchars($tags, ENT_QUOTES) . '</td>' .
                    '<td>' . htmlspecialchars($concept->getActiveY(), ENT_QUOTES) . '</td>' .
                    '<td>' . htmlspecialchars($concept->getWg2Link(), ENT_QUOTES) . '</td>' .
                    '<td style="">' . htmlspecialchars($name, ENT_QUOTES) . '</td>' .
                    '<td><a href="' . $module->getUrl('index.php', true) . '&option=ttl&record=' . $recordId . '">' . strip_tags($concept->getConceptTitle()) . '</a></td>';
            ...
            if ($concept->getConceptFile() != "") {
                $file_concept = getFileLink($module, $pidsArray['PROJECTS'], $concept->getConceptFile(), '1', '', $secret_key, $secret_iv, $currentUser['record_id'], "");
            }
            $concepts_table .= '<td class="text-center col-1">' . $file_concept . '</td>';
...
$harmonist_perm_new_concept = ($currentUser['harmonist_perms___2'] == 1) ? true : false;
?>
...
if(array_key_exists('message', $_REQUEST)){
    if($_REQUEST['message'] == 'N'){
        ?>
        <div class="alert alert-success d-flex align-items-center col-12" style="border-color: #b2dba1 !important;" id="succMsgContainer">
            <i class="bi bi-check-circle-fill me-2"></i>
            Your concept has been successfully added.
        </div>
        <?php
    }
}
...
<div class="container">
    ...
    <?php if(empty($concepts)){
        echo '<div class="optionSelect"><div class="alert alert-warning text-center">No concepts found.</div></div>';
    }?>
        ...
        <div style="<?=$button_style;?>">
            ...
            if ($isAdmin && $settings['deactivate_concept_tracker___1'] != 1) {
                ...
                <div style="display: inline-block">
                    <a href="<?=$module->getUrl('hub/hub_concepts_tracker_spreadsheet.php', true)?>"class="btn btn-primary me-3"><i class="fa fa-arrow-down"></i> Concept Tracker</a>
                </div>
            <?php } ?>
            <?php if ($isAdmin || $harmonist_perm_new_concept) { ?>
                <div style="<?=$newconcept_btn_css?>">
                    <a href="#" onclick="$('#hub_new_concept').modal('show');" class="btn btn-success btn-md"><i class="fa fa-plus"></i> New Concept</a>
                </div>

                <!-- MODAL NEW CONCEPT -->
                <div class="modal fade" id="hub_new_concept" tabindex="-1" aria-labelledby="Codes" aria-hidden="true">
    ...
    <div class="mt-5" style="float:right;padding-bottom:5px;">
        <div class="form-check">
            <input type="checkbox" class="form-check-input" id="concept_active" name="concept_active" checked>
            <label class="form-check-label" for="concept_active">Active Concepts Only</label>
        </div>
    </div>
    <div class="optionSelect conceptSheets_optionMenu">
        ...
        <div style="float:right">
            ...
            <div style="float:left;padding-left:10px">
                <select class="form-select" name="selectWorkingGroup" id="selectWorkingGroup">
            ...
            <div style="float:left;padding-left:10px">
                <select class="form-select" name="selectTags" id="selectTags">
                    <option value="">Select All</option>
                    <?php

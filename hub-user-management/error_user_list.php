            ...
            $('#add_error_user_management').click(function (event) {
                ...
                $("#addUsersForm input:not([type=hidden])").each(function (el) {
                    ...
                    if(userName == undefined || userName == ""){
                        ...
                        userNameIdArray.push(this.id);
                    }
                });
                doesUserNameExist(userNameArray,userNameIdArray,errors, urlUserManagament);
            });
        });

        function doesUserNameExist(userNameArray,userNameIdArray,errors, urlUserManagament){
            $.ajax({
                method: "POST",
                url: <?=json_encode($module->getUrl('hub-user-management/doesUserNameExist_AJAX.php'))?>,
                data: {
                    userNameArray: userNameArray,
                    userNameIdArray: userNameIdArray
                }
            }).done(function(data) {
                let errorText = "";
                if(errors) {
                    errorText += "Username can't be blank.<br>";
                }
                for (let key in data) {
                    $('#'+key).addClass("error");
                    errorText += "User <strong>"+data[key]+"</strong> does not exist in REDCap.<br>";
                }
                if(errorText != ""){
                    $('#msgUserList').html(errorText);
                    $('#msgUserList').show();
                }else{
                   manageUserFromDataDownloads(urlUserManagament,'add');
                }
            });
        }
...
<div style="padding-top:15px;padding-left:15px;">
    <h3>Data Download Users Management - Resolve Permission Conflicts</h3>
</div>
<div style="padding-top:15px;padding-left:15px;padding-bottom: 60px;">
    These users are <b>missing the correct permissions </b>to use the Hub Data Downloads feature.<br>
    Click on each user to see the missing permissions.
</div>
<?php
$show = true;
...
<div id="addUsersForm" title="Add/Repair Data Downloaders" style="display:none;">
    <div class="alert alert-danger fade in col-md-12" style="display:none;" id="msgUserList"></div>
    <p>Review and fill any incomplete account matches.</p>
    ...
    <div class="modal-footer" style="border-top:0;padding-top: 30px;">

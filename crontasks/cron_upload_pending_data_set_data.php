...
$credentialsPath = $this->getProjectSetting('credentials-path', $pidsArray['PROJECTS']);
$awsFile = $this->getProjectSetting("aws-file", $pidsArray['PROJECTS']);

if (!empty($credentialsPath) && !empty($awsFile)) {
    $path = rtrim($credentialsPath, '/') . '/';
    $aws_credentials = $path . $awsFile;

    if (is_file($aws_credentials)) {

...
if(strtotime($settings['publications_lastupdate']) < $today || $settings['publications_lastupdate'] == "" || $isAdmin) {
    ...
    $groups = \REDCap::getData($pidsArray['GROUP'], 'json-array', null, ['group_abbr', 'record_id']);
    $group_abbr = array_column($groups, 'group_abbr', 'record_id');
    ...
    $table_array['data'] = [];
    if (!empty($concepts) || !empty($extra_outputs)) {
        $records = 0;
        if (!empty($concepts)) {
            foreach ($concepts as $concept) {
                ...
                if (!empty($output_year)) {
                    ...
                    //Add working groups
                    $wg_values = array_filter(array_unique([
                        $concept['wg_link'] ?? '',
                        $concept['wg2_link'] ?? '',
                        $concept['wg3_link'] ?? '',
                        $concept['wg4_link'] ?? '',
                    ...
                    $wg_badges = '';
                    foreach ($wg_values as $wg_id) {
                        if (!empty($group_abbr[$wg_id])) {
                            $wg_label = htmlentities($group_abbr[$wg_id]);
                            $wg_badges .= '<span class="badge bg-outline">' . $wg_label . '</span> ';
                        }
                    }

                    $concept_tags = implode(',', array_keys(array_filter($concept['concept_tags'], function($v) { return $v == 1; })));
                    ...
                    foreach ($output_year as $index => $value) {
                        ...
                        $file = '';
                        if (array_key_exists('output_file', $concept) && is_array(
                                $concept['output_file']
                            ) && array_key_exists(
                                $index,
                                $concept['output_file']
                            ) && $concept['output_file'][$index] !== "") {
                            $file = $concept['output_file'][$index];
                        }

                        $passthru_link = $moduleAux->resetSurveyAndGetCodes(
                            $pidsArray['HARMONIST'],
                            $concept['record_id'],
                            "outputs",
                            "",
                        ...
                        $survey_link = APP_PATH_WEBROOT_FULL . "/surveys/?s=" . $moduleAux->escape(
                                $passthru_link['hash']
                            );

                        $edit = '<a href="#" class="btn btn-outline-secondary" onclick="editIframeModal(\'hub_edit_pub\',\'redcap-edit-frame\',\'' . $survey_link . '\');"><em class="fa fa-pencil"></em></a>';

                        $badge = "";
                        $output_type = "";
                        if (is_array($concept['output_type']) && array_key_exists(
                                $index,
                                $concept['output_type']
                            ) && !empty($concept['output_type'][$index])) {
                            ...
                            if (array_key_exists($concept['output_type'][$index], $abstracts_publications_type)) {
                                $output_type = htmlentities(
                                    $abstracts_publications_type[$concept['output_type'][$index]]
                                );
                            }
                        }

                        $authors = "";
                        if (is_array($concept['output_authors']) && array_key_exists(
                                $index,
                                $concept['output_authors']
                            )) {
                        ...
                        $table_aux = array();
                        $table_aux['concept'] = '<a href="' . $moduleAux->getUrl(
                                'index.php',
                                true
                            ) . '&pid=' . $pidsArray['PROJECTS'] . '&option=ttl&record=' . $concept['record_id'] . '">' . htmlentities(
                                $concept['concept_id']
                            ) . '</a>';
                        ...
                        $table_aux['title'] = '<span class="badge badge-pill ' . $badge . '">' . $output_type . '</span><span style="display:none">.</span> <strong>' . htmlentities(
                                $title
                            ) . '</strong><span style="display:none">.</span> '.$wg_badges.'</br><span class="abstract_text">' . htmlentities(
                                $authors
                            ) . '</span>';
                        $table_aux['available'] = $available;
                        $table_aux['file'] = $file;
                        $table_aux['edit'] = $edit;
                        $table_aux['group'] = implode(',', $wg_values);
                        $table_aux['tags'] = $concept_tags;
        ...
        if (!empty($extra_outputs)) {
            ...
            foreach ($extra_outputs as $output) {
                ...
                if ($output['producedby_region'] == 2) {
                    ...
                    if ($output['producedby_region'] == 1 && !empty($output['lead_region'])) {
                        $regionData = \REDCap::getData(
                            $pidsArray['REGIONS'],
                            'json-array',
                            array('record_id' => $output['lead_region']),
                            array('region_code')
                        );
                        $my_region = "";
                        if (!empty($regionData) && array_key_exists(0, $regionData) && array_key_exists(
                                'region_code',
                                $regionData[0]
                            )) {
        ...
        $json = json_encode(
            array(
                array(
                    'record_id' => 1,
                    'publications_json' => $docId,
                    'publications_lastupdate' => date("Y-m-d H:m:s")
                )
            )
        );

...
use Carbon\Carbon;
use Vanderbilt\HarmonistHubPublicExternalModule\ArrayFunctions;
    ...
    if($edoc != ""){
        ...
        while ($row = $q->fetch_assoc()) {
            ...
            if($type == "img"){
                ...
                $file = $module->getSafePath(EDOC_PATH.$row['stored_name'],EDOC_PATH);
            }else if($type == "imgpdf"){
...
function getFileLink($module, $project_id, $edoc, $option, $outer,$secret_key,$secret_iv,$user,$lid){
    ...
    if($edoc != "" and is_numeric($edoc)){
        ...
        while ($row = $q->fetch_assoc()) {
            ...
            $file_url = $module->getUrl("downloadFile.php", true)."&code=".$module->escape($download);
            if($option == 'link') {
                $file_row = $file_url;
            }elseif($option == 'ext'){
                $file_row = strtoupper($row['file_extension']);
            }elseif($option == ''){
                $icon = getFaIconFile($row['file_extension']);
                $file_row = "<a href='".$file_url."' target='_blank'>". $icon . $module->escape($row['doc_name'])."&nbsp;".ProjectData::formatBytes($row['doc_size'])."</a>";
            }else{
...
function getPeopleName($pidPeople, $people_id, $option=""){
    if(!empty($people_id) && !empty($pidPeople)){
        ...
        $name = trim(arrayKeyExistsReturnValue($people,['firstname']).' '.arrayKeyExistsReturnValue($people,['lastname']));
        if($option == "email"){
            $name = '<a href="mailto:'.$people['email'].'">'.trim(arrayKeyExistsReturnValue($people,['firstname']).' '.arrayKeyExistsReturnValue($people,['lastname'])).'</a>';
        }
        return $name;
    }
    return "";
...
function getRegionsSortedByVoteOrder($regions) {
    $hasVoteOrder = array_filter($regions, function($r) {
        return !empty($r['voteregion_order']);
    });
    if (!empty($hasVoteOrder)) {
        ArrayFunctions::array_sort_by_column($regions, 'voteregion_order');
    } else {
        ...
        if ($vote_grid == '2' || ($isAdmin && $vote_grid == "0")) {
            ...
            foreach ($regions as $region) {
                ...
                if (
                    ...
                    $header_region .= '<th class="request_grid_icon ' . $selected . $small_screen_class . '" data-sortable="false">' . $region['region_code'] . '</th>';
 ...
 * @return string
 */
function getPrivateVotesHTML($region_response_status,$small_screen_class, $selected){
    $current_req_region = "";
    if ($region_response_status == 0) {
        // Not started
        $current_req_region .= '<td class="' . $selected . $small_screen_class . ' moz" style="text-align: center;">
            <span class="badge bg-light" data-bs-toggle="tooltip" title="Not Started">
        ...
        // In Progress
        $current_req_region .= '<td class="' . $selected . $small_screen_class . ' moz" style="text-align: center;">
            <span class="badge bg-warning" data-bs-toggle="tooltip" title="In Progress">
        ...
        // Complete
        $current_req_region .= '<td class="' . $selected . $small_screen_class . ' moz" style="text-align: center;">
            <span class="badge bg-info" data-bs-toggle="tooltip" title="Complete">
...
function getMixVotesHTML($commentReq, $region_vote_status,$region_response_status,$region_id,$req,$small_screen_class, $selected){
    $mix = false;
    ...
    if ($mix) {
        $current_req_region = '<td class="' . $selected . $small_screen_class . ' moz" style="text-align: center;">
            <span class="badge bg-secondary" data-bs-toggle="tooltip" title="Mix">
        ...
        if ($region_vote_status == "0") {
            $current_req_region = '<td class="' . $selected . $small_screen_class . ' moz" style="text-align: center;">
                <span class="badge bg-danger" data-bs-toggle="tooltip" title="Not Approved">
                    <i class="fa fa-times" aria-hidden="true"></i>
                </span>
            </td>';
        } else if ($region_vote_status == "1") {
            $current_req_region = '<td class="' . $selected . $small_screen_class . ' moz" style="text-align: center;">
                <span class="badge bg-success" data-bs-toggle="tooltip" title="Approved">
                    <i class="fa fa-check" aria-hidden="true"></i>
                </span>
            </td>';
            ...
            // Abstained
            $current_req_region .= '<td class="' . $selected . $small_screen_class . ' moz" style="text-align: center;">
                <span class="badge bg-secondary" data-bs-toggle="tooltip" title="Abstained">
 ...
 * @return string
 */
function getPublicVotesHTML($vote,$small_screen_class, $selected=""){
    $current_req_region = "";
    if ($vote == "1") {
        // Approved
        $current_req_region .= '<td class="' . $selected . $small_screen_class . ' moz" style="text-align: center;">
...
function getRequestHTML($module, $hubData, $pidsArray, $req, $commentReq, $request_type_label, $option, $vote_visibility, $vote_grid, $req_type){
    ...
    $width = [0 => "style='width:70px'", 1 => "style='width:70px'", 2 => "style='width:60px'", 3 => "style='width:120px'"];
    $button_text = "Respond";
    $button_icon = "fa-share";
    if ($req_type == 'home') {
        $width = [0 => "style='width:100px'", 1 => "style='width:150px'", 2 => "style='width:450px'"];
        $button_text = "View";
        $button_icon = "fa-eye";
    } else if ($req_type == 'archive') {
        $width = [0 => "style='width:80px'", 1 => "style='width:150px'", 2 => "style='width:590px'"];
        $button_text = "View";
        $button_icon = "fa-eye";
    }

    $current_req = '<tr>
                    <td ' . $width[0] . ' class="ps-3"><span class="' . $class . '">' . $req['due_d'] . '</span></td>
                    <td ' . $width[1] . '>
    ...
    $current_req .= '<td ' . $width[2] . ' class="d-none d-sm-table-cell">
                        <a href="' . $module->getUrl('index.php', true) . '&option=hub&record=' . $req['request_id'] . $type . '">' . $text . $req['request_title'] . '</a>
                    </td>';
    ...
    if ($option != '2') {
        ...
        $view_all_votes = "";
        if ($vote_grid == '2' || ($vote_grid == '0' && $isAdmin)) {
            $url = $module->getUrl("hub/hub_requests_view_all_votes_AJAX.php", true);
            $view_all_votes = '<div><a href="#" onclick="viewAllVotes(' . $req['request_id'] . ',\'' . $url . '\',\'' . $redcapCsrfToken . '\');" class="btn btn-success btn-sm" style="margin-bottom: 7px;">
                                    <span class="fa fa-folder-open"></span> All votes
                             </a></div>';
        }
        if ($vote_visibility == '3') {
            $url = $module->getUrl("hub/hub_requests_view_mixed_votes_AJAX.php", true);
            $view_all_votes .= '<div><a href="#" onclick="viewMixedVotes(' . $req['request_id'] . ',' . $current_user['person_region'] . ',\'' . $url . '\',\'' . $redcapCsrfToken . '\');" class="btn btn-success btn-sm" style="margin-bottom: 7px;">
                                    <span class="fa fa-folder-open"></span> Vote Details
                                </a></div>';
        }
        if ($option == 0) {
            if ($req_type == 'archive') {
                ...
                if ($current_user['harmonist_regperm'] == 1) {
                    $current_req .= '<td ' . $width[3] . '>' . $view_all_votes . '<div>
                                        <a href="' . $module->getUrl('index.php', true) . '&option=hub&record=' . $req['request_id'] . '" class="btn btn-link btn-sm">
                                            <span class="fa fa-eye"></span> View
                                        </a></div>';
                } else {
                    $current_req .= '<td ' . $width[3] . '>' . $view_all_votes . '<div>
                                        <a href="' . $module->getUrl('index.php', true) . '&option=hub&record=' . $req['request_id'] . '" class="btn btn-link btn-sm">
                                            <span class="fa ' . $button_icon . '"></span> ' . $button_text . '
                                        </a></div>';
                }
            }
        } else {
            $current_req .= '<td ' . $width[3] . '>' . $view_all_votes . '<div>
                                <a href="' . $module->getUrl('index.php', true) . '&option=hub&record=' . $req['request_id'] . '" class="btn btn btn-link btn-sm actionbutton">
                                    <span class="fa fa-eye"></span> View/Edit
                                </a></div>';
        }
    } else {
        $reviewer = '';
        ...
        $current_req .= '<td width="200px"><div>
                            <a href="' . $survey_link . '" class="btn btn-link btn-sm actionbutton" target="_blank">
                                <i class="fa fa-eye fa-fw" aria-hidden="true"></i> Check Submission
                            </a></div>';

        $passthru_link = $module->resetSurveyAndGetCodes($pidsArray['RMANAGER'], $req['request_id'], "admin_review", "");
        $survey_link = $module->escape(APP_PATH_WEBROOT_FULL . "/surveys/?s=" . $passthru_link['hash']);
        $current_req .= '<div>
                            <a href="#" onclick="editIframeModal(\'hub_process_survey\',\'redcap-edit-frame-admin\',\'' . $survey_link . '\');" class="btn btn-link btn-sm open-codesModal" style="margin-top: 7px;">
                                <i class="fa fa-calendar fa-fw" aria-hidden="true"></i> Change Status
                            </a></div>';
    }
...
function getArchiveHTML($module,$pidsArray,$req,$request_type_label,$person_region, $vote_visibility){
    ...
    $region = arrayKeyExistsReturnValue(\REDCap::getData($pidsArray['REGIONS'], 'json-array', array('record_id' => $req['contact_region']),null,array('region_code'),null,false,false,false,"[showregion_y] = 1"), [0]);

    $current_req .= '</td>
                    <td>'.arrayKeyExistsReturnValue($req, ['contact_name']).'</td>
                    <td>'.arrayKeyExistsReturnValue($region, ['region_code']).'</td>
                    <td>'.arrayKeyExistsReturnValue($req, ['request_title']).'</td>';
    ...
    $current_req .= '<td width="130px"><div><a href="'.$module->getUrl('index.php', true).'&record='.$req['request_id'].'&option=hub'.'" class="btn btn-link btn-xs actionbutton"><em class="fa fa-eye"></em> View/Edit</a></div>';
    $current_req .= '</td>';
...
function getRequestVoteIcon($commentReq, $current_req_region, $vote_grid, $person_region ,$record_id, $vote_visibility, $req, $current_user){
    ...
    $regionResponseStatus = arrayKeyExistsReturnValue($req,['region_response_status', $instance]);
    $regionVoteStatus = arrayKeyExistsReturnValue($req,['region_vote_status', $instance]);
    if($vote_visibility == "" || $vote_visibility =="1") {
        //PRIVATE VOTES
        $current_req_region .= getPrivateVotesHTML($regionResponseStatus, $small_screen_class, $selected);
    }else if($vote_visibility =="3"){
        //MIX VOTES
        $current_req_region .= getMixVotesHTML($commentReq, $regionVoteStatus,$regionResponseStatus, $record_id, $req, $small_screen_class, $selected);
    }else{
        if ($regionResponseStatus == "2") {
            //PUBLIC VOTES
            $current_req_region .= getPublicVotesHTML($regionVoteStatus,$small_screen_class, $selected);
        }else{
            $current_req_region .= getPrivateVotesHTML($regionResponseStatus,$small_screen_class, $selected);
        }
    }
...
function getNumberOfDaysLeftButtonHTML($date_deadline, $region_response_status,$button_style, $date_option, $due=""){
    if(!empty($date_deadline)) {
        ...
        $array_dates = array('text' => '<span class="'.$date_color_text.'">'.$due_date.'</span>','button' =>'<span class="badge rounded-pill '.$date_color_button.'" style="'.$button_style.'">'.$number_days.$until_due.'</span>');
        return $array_dates;
    }else{
...
function getDataCallHeader($hubData, $pidRegions, $person_region, $vote_grid, $option=""){
    ...
    if($vote_grid == '2' || $vote_grid == '0') {
        ...
        foreach ($regions as $region) {
            ...
            if ($currentUser['person_region'] == $instance) {
                $selected = 'selected-region';
            } else {
                $selected = '';
            }

            $header_region .= '<th class="request_grid_icon ' . $selected . '" data-sortable="false" style="width:40px;text-align: center">' . $region['region_code'] . '</th>';
...
function getDataCallRow($module, $pidsArray, $sop,$isAdmin,$current_user,$secret_key,$secret_iv,$vote_grid,$type,$harmonist_perm=""){
    ...
    $peopleData = \REDCap::getData($pidsArray['PEOPLE'], 'json-array', array('record_id' => $sop['sop_datacontact']), array('person_region','email','firstname','lastname'));
    $people = $peopleData[0] ?? [];

    $region_code = '';
    if (!empty($people['person_region'])) {
        $regionData = \REDCap::getData($pidsArray['REGIONS'], 'json-array', array('record_id' => $people['person_region']), array('region_code'));
    ...
    if ($type == "s" || $type == "a") {
        if ($type == "a") {
            ...
            $btn_width = "150px";
            $buttons = '<div><a href="#" onclick="confirmDataUpload(\'' . $sop['sop_concept_id'] . '\',\'' . $current_user['record_id'] . '\',\'' . $concept_id . '\',\'' . $sop['record_id'] . '\');" class="btn btn-primary btn-sm">Upload Data</a></div>';
            if ($current_user['allowgetdata_y___1'] == "1" || $current_user['harmonistadmin_y'] == '1') {
                $buttons .= '<div style="padding-top: 8px"><a href="#" onclick="changeStatus(\'' . $current_region_status . '\',\'' . $sop['record_id'] . '\',\'' .$personRegion . '\',\'' . htmlspecialchars(arrayKeyExistsReturnValue($sop, ['data_response_notes', $personRegion])) . '\',\'' . htmlspecialchars(arrayKeyExistsReturnValue($sop, ['region_update_ts', $personRegion])) . '\',\'modal-data-change-status\')" class="btn btn-outline-secondary btn-sm">Change Status</a></div>';
            }
        }

        $url = "&type=s";

        $data .= "<td><div class='text-center'>" . $array_dates['text'] . "</div><div>" . $array_dates['button'] . "</div></td>";
    } else if ($type == "p") {
        if ($isAdmin || $harmonist_perm || $sop['sop_hubuser'] == $current_user['record_id'] || $sop['sop_creator'] == $current_user['record_id'] || $sop['sop_creator2'] == $current_user['record_id'] || $sop['sop_datacontact'] == $current_user['record_id']) {
            $buttons .= '<div><a href="'.$module->getUrl('index.php', true).'&option=ss1&record=' . $sop['record_id'] . '&step=3'.'" class="btn btn-outline-secondary btn-sm" target="_blank"><i class="fa fa-edit text-primary" aria-hidden="true"></i> Edit</a></div>';
        }
        if ($isAdmin || $harmonist_perm) {
            $buttons .= '<div style="padding-top: 8px"><a href="#" onclick="confirmMakePrivate(\'' . $sop['record_id'] . '\')" class="btn btn-outline-secondary btn-sm"><i class="fa fa-thumb-tack text-default" aria-hidden="true"></i> Private</a></div>';
        }

        $status_row = "<td style='width: 150px'><div>" . $sop['sop_updated_dt'] . "</div></td>";
    } else if ($type == 'm') {
        $buttons = '';
        if ($isAdmin || $harmonist_perm || $sop['sop_hubuser'] == $current_user['record_id'] || $sop['sop_creator'] == $current_user['record_id'] || $sop['sop_creator2'] == $current_user['record_id'] || $sop['sop_datacontact'] == $current_user['record_id']) {
            $buttons .= '<div><a href="'.$module->getUrl('index.php', true).'&option=ss1&record=' . $sop['record_id'] . '&step=3'.'" class="btn btn-outline-secondary btn-sm" target="_blank"><i class="fa fa-edit text-primary" aria-hidden="true"></i> Edit</a></div>';
        ...
        if ($sop['sop_visibility'] == '2') {
            ...
            $buttons .= '<div><a href="#" onclick="confirmMakePrivate(\'' . $sop['record_id'] . '\')" class="btn btn-outline-secondary btn-sm" style="margin-top: 7px;"><i class="fa fa-thumb-tack text-default" aria-hidden="true"></i> Private</a></div>';
            ...
            $buttons .= '<div><a href="#" onclick="editIframeModal(\'sop-make-public\',\'redcap-edit-frame-make-public\',\'' . $survey_link . '\');" class="btn btn-outline-secondary btn-sm" style="margin-top: 7px;"><i class="fa fa-paper-plane text-success" aria-hidden="true"></i> Route</a></div>';
        }
        $buttons .= '<div><a href="#" onclick="deleteDataRequest(\'' . $sop['record_id'] . '\')" style="cursor: pointer;margin-top: 7px;" class="btn btn-outline-secondary btn-sm" title="Delete Data Request"><i class="fa fa-trash-o text-danger" aria-hidden="true"></i> Delete</a></div></td>';
    ...
    if(arrayKeyExists($sop, 'sop_tags')){
        ...
        if(!empty($selectedTags)){
            ...
            $sopTags = implode('', array_map(function ($tag) use ($selectedTagsLabel) {
                return "<div class='d-inline-block mt-1' style='padding-right: 5px'><span class='badge badge-draft rounded-pill'>" . htmlspecialchars($selectedTagsLabel[$tag]) . "</span></div>";
            }, $selectedTags));
        }
    }

    $data .=    "<td style='width: 800px'><div><strong>" . $concept_id . "</strong> ".$sop_visibility."</div><div>" . $concept_title . "</div><div>".$sopTags."<div class='d-inline-block'><em>Draft ID: ".$sop['record_id']."</em></div></div><div><a href='".$module->getUrl("index.php", true)."&option=sop&record=".$sop['record_id'].$url."'>Data Request </a> | <a href='".$module->getUrl("index.php")."&NOAUTH&option=ttl&record=".$sop['sop_concept_id']."'>".$concept_id." Concept</a>".$file_data."</div></td>" .
        "<td style='width: 150px'>" . $contact_person . "</td>" .
        $status_row.
        "<td style='width: ".$btn_width."'>" . $button_votes.$buttons . "</td>" .
        "</tr>";
    ...
    if($option == "1"){
        ...
        if($sop['sop_status'] == "1"){
            if ($sop['sop_closed_y'] != '1') {
                $sop_closed_y = '<span class="badge rounded-pill label-retrieve">Open</span>';
            } else if ($sop['sop_closed_y'] == '1') {
                $sop_closed_y = '<span class="badge rounded-pill label-default_dark">Closed</span>';
            }
        }
        ...
        $sop_status = '<span class="badge rounded-pill '.$status.'">'. $sop_status[$sop['sop_status']].'</span>&nbsp;&nbsp;';
...
function getDataCallStatusIcons($status){
    switch ($status) {
        case "0":
            $status_icons = '<span class="badge bg-light" title="Not Started"><i class="fa fa-times" status="' . $status . '"></i></span>';
            break;
        case "1":
            $status_icons = '<span class="badge bg-warning" title="Partial Data"><i class="fa fa-wrench" status="' . $status . '"></i></span>';
            break;
        case "2":
            $status_icons = '<span class="badge bg-success" title="Complete Data"><i class="fa fa-check" status="' . $status . '"></i></span>';
            break;
        case "3":
            $status_icons = '<span class="badge bg-secondary" title="Data Not Available"><i class="fa fa-ban" status="' . $status . '"></i></span>';
            break;
        case "4":
            $status_icons = '<span class="badge bg-secondary" title="Region Not Requested"><i class="fa fa-times" status="' . $status . '"></i></span>';
            break;
        case "9":
            $status_icons = '<span class="badge bg-other" title="Other Status"><i class="fa fa-question" status="' . $status . '"></i></span>';
            break;
        default:
            $status_icons = '<span class="badge bg-light" title="Not Started"><i class="fa fa-times" status="0"></i></span>';
    }
    return $status_icons;
}
...
function generateTableArray($module, $pidDataModel){
    ...
    foreach( $recordsTable as $data ){
        ...
        if(array_key_exists('variable_order',$data) && is_array($data['variable_order'])){
 ...
 */
function generateTablesHTML_steps(array $pidsArray, array $dataTable): string
{
    $pidCodeList = $pidsArray['CODELIST'] ?? null;

    $desUrlBase = APP_PATH_WEBROOT_FULL
        . "external_modules/?prefix=data-model-browser&page=browser&NOAUTH&pid="
        . ($pidsArray['DES'] ?? '');

    // Cache code list lookups to avoid calling REDCap::getData repeatedly
    $codeListCache = []; // [record_id => ['code_format'=>..., 'code_list'=>...]]

    $out = [];

    // NOTE: your $dataTable contains both rows + metadata sometimes; keep original behavior
    $formatLabels = $dataTable['data_format_label'] ?? [];

    foreach ($dataTable as $data) {
        if (!is_array($data) || empty($data['record_id']) || empty($data['variable_order']) || !is_array($data['variable_order'])) {
            continue;
        }

        $recordId = $data['record_id'];
        $varNames = $data['variable_name'] ?? [];
        $descriptions = $data['description'] ?? [];
        $variableStatusArr = $data['variable_status'] ?? [];

        $headerOpened = false;
        $lastVarRecordId = $recordId . '_1';

        foreach ($data['variable_order'] as $vid => $_) {
            $vidKey = (string)$vid;

            $recordVarName = $varNames[$vidKey] ?? ($varNames[''] ?? '');
            $recordVarNameId = ($vidKey === '' || $vidKey === '0') ? ($recordId . '_1') : ($recordId . '_' . $vidKey);
            $lastVarRecordId = $recordVarNameId;

            if (!$headerOpened) {
                $tableDraftClass = 'bg-light';
                $tableDraftText = '';

                if (array_key_exists('table_status', $data)) {
                    $isDraft = ((string)$data['table_status'] === '0');
                    $tableDraftClass = $isDraft ? 'des_draft_header' : 'bg-light';
                    $tableDraftText  = $isDraft ? '<span class="text-danger fst-italic">(DRAFT)</span>' : '';
                }

                // Open card + header
                $out[] =
                    '<div class="card preview" style="display:none;" record_id="' . $recordVarNameId . '">' .
                    '<div class="card-header fw-normal" style="display:none;" record_id="' . $recordVarNameId . '" parent_table_header="' . $recordId . '">' .
                    '<h6 class="mb-0">' .
                    '<span class="fs-5 ' . $tableDraftClass . '">' .
                    '<a href="' . $desUrlBase . '&tid=' . $recordId . '&option=variables" target="_blank" class="badge text-decoration-none des-' . ($data['table_category'] ?? '') . '">' .
                    ($data['table_name'] ?? '') .
                    '</a>' .
                    '</span>' .
                    $tableDraftText .
                ...
                if (!empty($data['text_top'])) {
                    $out[] = '<h6 class="border-0 fst-italic" style="display:none" parent_table_header="' . $recordId . '">' . htmlspecialchars($data['text_top'], ENT_QUOTES) . '</h6>';
                }

                // IMPORTANT: close ONLY the card-header here (do not close table-no-borders here)
                $out[] = '</div>';

                // Open table wrapper + table + thead + tbody
                $tableDomId = 'PreviewTable_' . $recordId;

                $out[] =
                    '<div class="table-no-borders">' .
                    '<table class="table PreviewTable step_4_table fw-normal" parent_table="' . $recordId . '" id="' . $tableDomId . '">' .
                    '<thead>' .
                    '<tr style="display:none;" parent_table_header="' . $recordId . '">' .
                    '<th class="p-2" style="width:20%"><strong>Field</strong></th>' .
                    '<th class="p-2" style="width:20%"><strong>Format</strong></th>' .
                    '<th class="p-2"><strong>Description</strong></th>' .
                    '</tr>' .
                    '</thead>' .
                    '<tbody>';
            ...
            // Variable status handling
            $status = $variableStatusArr[$vidKey] ?? null;

            // Deprecated: skip entirely
            if ((string)$status === '2') {
                continue;
            }
            ...
            $variableText = '';
            if ((string)$status === '0') { // draft variable
                $cellStyle = 'background-color: #ffffe6;';
                $rowStyle = $cellStyle . 'display:none;';
                $variableText = "<span class='text-danger fw-bold'>DRAFT</span><br/>";
            }

            // Format label
            $dataFormat = '';
            $dataFormatKey = $data['data_format'][$vidKey] ?? null;
            if ($dataFormatKey !== null && isset($formatLabels[$dataFormatKey])) {
                $dataFormat = $formatLabels[$dataFormatKey];
            }

            $hasCodes = (string)($data['has_codes'][$vidKey] ?? '0');

            if ($hasCodes === '0') {
                $codeText = $data['code_text'][$vidKey] ?? '';
                if ($codeText !== '') {
                    $dataFormat .= "<br/>" . htmlspecialchars($codeText, ENT_QUOTES);
                }
            } elseif ($hasCodes === '1') {
                $codeRef = $data['code_list_ref'][$vidKey] ?? '';
                if ($codeRef !== '' && $pidCodeList) {
                    if (!isset($codeListCache[$codeRef])) {
                        $res = \REDCap::getData(
                            $pidCodeList,
                            'json-array',
                            ['record_id' => $codeRef],
                            ['code_format', 'code_list']
                        );
                        ...
                        $codeListCache[$codeRef] = [
                            'code_format' => $res['code_format'] ?? null,
                            'code_list'   => $res['code_list'] ?? null,
                        ];
                    }

                    $codeformat = $codeListCache[$codeRef];
                    $codeFormatType = (string)($codeformat['code_format'] ?? '');

                    if ($codeFormatType === '1') {
                        $fallback = $data['code_text'][$vidKey] ?? '';
                        $rawList = $codeformat['code_list'] ?? '';
                        $codeOptions = ($rawList === '' || $rawList === null) ? $fallback : explode(' | ', $rawList);

                        if (!empty($codeOptions) && (string)($codeOptions[0] ?? '') !== '') {
                            $dataFormat .= "<div class='ps-3'>";
                            foreach ((array)$codeOptions as $option) {
                                $dataFormat .= htmlspecialchars($option, ENT_QUOTES) . "<br/>";
                            }
                            $dataFormat .= "</div>";
                        }
                    } elseif ($codeFormatType === '3') {
            ...
            // Description
            $description = $descriptions[$vidKey] ?? ($descriptions[''] ?? '');
            $extra = $data['description_extra'][$vidKey] ?? '';
            if ($extra !== '') {
                $description .= "<br/><em>" . htmlspecialchars($extra, ENT_QUOTES) . "</em>";
            }

            $out[] =
                '<tr record_id="' . $recordVarNameId . '" style="' . $rowStyle . '">' .
                '<td class="p-2" style="' . $cellStyle . '">' .
                '<a href="' . $desUrlBase . '&tid=' . $recordId . '&vid=' . $vidKey . '&option=variableInfo" target="_blank" class="text-decoration-none">' .
                $recordVarName .
                '</a>' .
                '</td>' .
                '<td class="p-2" style="width:160px;' . $cellStyle . '">' .
                $dataFormat .
                '</td>' .
                '<td class="p-2" style="' . $cellStyle . '">' .
        ...
        if ($headerOpened) {
            // Close tbody + table + table wrapper + card
            $out[] =
                '</tbody>' .
                '</table>' .
                '</div>' . // close table-no-borders
                '<span record_id="' . $lastVarRecordId . '" style="display:none"></span>' .
                '</div>';  // close card

            if (!empty($data['text_bottom'])) {
                $out[] = '<div class="border-0 fst-italic" style="display:none;" parent_table="' . $recordId . '">' . htmlspecialchars($data['text_bottom'], ENT_QUOTES) . '</div>';
            }
        }
    }
    return implode('', $out);
            ...
            foreach ($fieldsSelected as $field) {
                ...
                if ($data['record_id'] == $field) {
                    ...
                    if($found == false){
                        ...
                        $htmlHeader .= '<table class="selectedTabledHtml" style="border-collapse: collapse;width: 100%;border:1px solid #000;">
                        <tr style="border:1px solid #000;'.$table_draft_tdcolor.'">
                            <td style="border:1px solid #000;padding: 5px;width:30%">Field</td>
                            <td style="border:1px solid #000;padding: 5px">Format</td>
                    ...
                    $tableHtml .='<tr record_id="'.$data["record_id"].'" '.$variable_status.'>
                                <td style="border:1px solid #000;padding: 5px"><a href="'.$desUrl.'tid='.$data['record_id'].'&vid='.$id.'&option=variableInfo" target="_blank" style="text-decoration:none">'.$record_varname.'</a></td>
                                <td style="border:1px solid #000;width:160px;padding: 5px">';
                    ...
                    if ($data['has_codes'][$id] == '0') {
                        ...
                        if(!empty($data['code_list_ref'][$id])){
                            $codeformat = \REDCap::getData($pidCodeList, 'json-array',array('record_id' => $data['code_list_ref'][$id]),array('code_format','code_list','code_file'))[0];
                    ...
                    $tableHtml .= $dataFormat . '</td>
                        <td style="border:1px solid #000;padding: 5px">'.$variable_text.$description. '</td>
                    </tr>';
                }
            }
            if($found) {
    ...
    if(!empty($csv)) {
        ...
        foreach ($csv as $header => $content) {
            ...
            foreach ($content as $col => $value) {
                ...
                if ($header == 0) {
                    $htmlCodes .= '<td style="border:1px solid #000;">' . $col . '</td>';
                } else {
                    $htmlCodes .= '<td style="border:1px solid #000;">' . $value . '</td>';
                }
            }
            $htmlCodes .= '</tr>';
        }

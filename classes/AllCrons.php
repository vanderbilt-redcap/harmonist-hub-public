...
class AllCrons
{
    ...
    public static function runCronMonthlyDigest($module, $pidsArray, $requests, $requests_hub, $sops, $settings, $email = false)
    {
        ...
        foreach ($sops as $sop){
            if((!array_key_exists('sop_closed_y',$sop) || $sop['sop_closed_y'] != "1") && $sop['sop_due_d'] != ""){
                ...
                $email_req .= "<div style='padding: 3px;'><a href='" . $module->getUrl("index.php")."&NOAUTH&pid=" . $pidsArray['DATAMODEL'] . "&option=hub&record=" . arrayKeyExistsReturnValue($sop,['request_id']) . "' target='_blank' alt='concept_link'>" . arrayKeyExistsReturnValue($sop,['request_title']). "</a></div>";
                $data_contact = getPeopleName($pidsArray['PEOPLE'], $sop['sop_datacontact']);
                $sop_creator = getPeopleName($pidsArray['PEOPLE'], $sop['sop_creator']);
                $sop_creator2 = getPeopleName($pidsArray['PEOPLE'], $sop['sop_creator2']);
                $sop_people = $sop_creator;

                if (!empty($creator)) {
                    // Add second creator's name if it exists
                    if (!empty($creator2['record_id'])) {
                        $sop_people .= ", " . $sop_creator2;
                    }

                    // Add data contact's name if both creator2 and datacontact records exist
                    if (!empty($creator2['record_id']) && !empty($datacontact['record_id'])) {
                ...
                $email_req .= "<div style='padding: 3px;'><a href='".$module->getUrl("index.php")."&NOAUTH&pid=".$pidsArray['PROJECTS']."&option=sop&record=".arrayKeyExistsReturnValue($sop,['record_id']). "'>" . arrayKeyExistsReturnValue($sop,['sop_name']) . "</a></div>";

                $votes = [];
                foreach ($regions as $region){
                    $regionId = arrayKeyExistsReturnValue($region,['record_id']);
                    $dataResponseStatus = arrayKeyExistsReturnValue($sop,['data_response_status',$regionId]);
                    if($dataResponseStatus == "1" || $dataResponseStatus == "2") {
        ...
        if($email) {
            if ($settings['hub_subs_monthly_digest'] != "") {
                ...
                foreach ($emails as $email) {
                    sendEmail($email, REDCapManagement::DEFAULT_NO_REPLY_EMAIL_ADDRESS, $settings['accesslink_sender_name'], $subject, $email_req, "Not in database", "Monthly Digest", $pidsArray['RMANAGER']);
                }
            }
        }
        $message['code_test'] = 1;
    ...
    public static function runCronReqFinalizedNotification($module, $pidsArray, $request, $settings, $email = false)
    {
        ...
        \REDCap::logEvent("runCronReqFinalizedNotification in EM", "record: ".$request['request_id'], null, $request['request_id'], $event_id, $pidsArray['RMANAGER']);

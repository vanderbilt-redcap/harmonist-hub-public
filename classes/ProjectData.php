...
class ProjectData
{
        ...
        if (is_array($data_dictionary_settings) && !empty($data_dictionary_settings)) {
            foreach ($data_dictionary_settings as $row) {
                if ($row['field_annotation'] !== "" && strpos($row['field_annotation'], "@DEFAULT") !== false) {
                    preg_match('/@DEFAULT=(["\'])(.*?)\1/', $row['field_annotation'], $matches);
                    if (isset($matches[2])) {
                        $default_value[$project_id][$row['field_name']] = $matches[2];
                    }
                }
            }
        }
    ...
    public static function checkIfDESIsEnabled($module, $pidsArray, $deactivateDatahub):bool
    {
        # Check if:
        # The Data Hub is activated
        # The DES pid exists
        # The DES Project is deleted
        # The DES module is activated
        if($deactivateDatahub != "1") {
            if (!array_key_exists("DES", $pidsArray)) {
                return false;
            } elseif (self::isProjectDeleted($module, $pidsArray['DES'])) {
                return false;
            } elseif (!$module->isModuleEnabled("data-model-browser", $pidsArray['DES'])) {
                return false;
            }
        }
        return true;
    ...
    public static function checkIfMissingProjects($pidsArray):array
    {
        # Check for any missing projects
       $projectsArray = [];
       $constants = REDCapManagement::getProjectsConstantsArray();
       foreach ($constants as $index => $constant) {
           if(arrayKeyExistsReturnValue($pidsArray,[$constant]) == null){
               $projectsArray[$index] = $constant;
           }
       }
       return $projectsArray;
    }

    public static function isProjectDeleted($module, $projectId):bool
    {
        $q = $module->query(
            "SELECT project_name FROM redcap_projects where project_id = ? AND (date_deleted IS NOT NULL OR completed_time IS NOT NULL)",
            [$projectId]
        );
        if($q->num_rows > 0){
            return true;
    ...
    {
         if (!$module->isModuleEnabled("harmonist-hub-public", $pidConstant)){
             if ($saveSettings) {
                 $module->enableModule($pidConstant, "harmonist-hub-public");
                 REDCap::logEvent(
                     "Hub Updates: harmonist-hub-public EM enabled on  " . $constant . " (PID #" . $$pidConstant . ")",
    ...
    public static function getCompletionTime($instrumentCompleteData, $instrumentTimestampData):string
    {

        $completion_time = ($instrumentCompleteData ?? null) == '2'
            ? $instrumentTimestampData ?? null
            : null;

        // validate timestamp (expects "Y-m-d H:i:s")
        $date = null;
        if (is_string($completion_time) && $completion_time !== '') {
            $date = \DateTime::createFromFormat('Y-m-d H:i:s', $completion_time);
            ...
            if ($date === false || $warningCount > 0 || $errorCount > 0
                || $date->format('Y-m-d H:i:s') !== $completion_time) {
                $date = null; // invalid
            }
        }

        if ($date === null) {
            $date = new \DateTime(); // add Today's date
        }

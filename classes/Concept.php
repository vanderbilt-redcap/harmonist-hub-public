...
class Concept extends Model
{
    ...
    public function __construct($conceptData, HarmonistHubPublicExternalModule $module, $pidsArray, $authorshipLimit = null, $decorateConcept = false)
    {
    ...
    public function setWgLink($wgLink): string
    {
        $this->wgLink = $wgLink;
    }
    ...
    public function setWg2Link($wg2Link): string
    {
        $this->wg2Link = $wg2Link;
    }
    ...
    public function setWg3Link($wg3Link): string
    {
        $this->wg3Link = $wg3Link;
    }

    public function getWg3Link(): string
    {
        return $this->wg3Link;
    }
    ...
    private function hydrateConcept()
    {
        ...
        $this->wg3Link = $this->conceptData['wg3_link'];
        $this->wg4Link = $this->conceptData['wg4_link'];
        ...
        $this->startYear = $this->conceptData['start_year'];
    ...
    public function getWgLinkByIndex(int $i): ?string
    {
        return match ($i) {
            1 => $this->getWgLink(),
            2 => $this->getWg2Link(),
    ...
    private function decorateStatus(): void
    {
        if ($this->activeY == "Y") {
            $active = "Active";
            $activeColorButton = "bg-success";
        } else {
            $active = "Inactive";
            $activeColorButton = "bg-danger";
        }

        $revised = "";
        if ($this->revisedY == '1') {
            $revised = '<span class="badge rounded-pill badge-revised">Revised</span>';
        }

        $this->status = '<span class="badge rounded-pill ' . $activeColorButton . '">' . $active . '</span> ' . $revised;
    }

...
class WritingGroupModel extends Model
{
    ...
    private $totalWritingGroupMemberByResearch = [];

    public function __construct(HarmonistHubPublicExternalModule $module, $projectId, Concept $concept)
    {
    ...
    public function getTotalWritingGroupMemberByResearch(): array
    {
        return $this->totalWritingGroupMemberByResearch;
    }

    public function setTotalWritingGroupMemberByResearch(array $totalWritingGroupMemberByResearch): void
    {
        $this->totalWritingGroupMemberByResearch = $totalWritingGroupMemberByResearch;
    }
    ...
    private function fetchWritingGroupByResearchGroup():void
    {
        ...
        $regions = $this->module->escape(\REDCap::getData($params));
        for ($i = 1; $i < ((int)$this->concept->getAuthorshipLimit() + 1); $i++) {
            foreach ($regions as $region) {
                ...
                $saveData = false;
                if(!array_key_exists($region_id, $this->totalWritingGroupMemberByResearch)) {
                    $this->totalWritingGroupMemberByResearch[$region_id] = 0;
                }
                ...
                if ($saveData) {
                    $this->totalWritingGroupMemberByResearch[$region_id] += 1;

...
class People extends Model
{
    ...
    public function __construct($peopleData, HarmonistHubPublicExternalModule $module, $pidsMapper)
    {
    ...
    private function addUserToDataDonwloadsProject(): void
    {
        if(!$this->isUserInDataDownloads()){
            $user_roles = HubREDCapUsers::getAllRoles($this->module, $this->getPidsArray()['DATADOWNLOADUSERS']);
            $roleId = $user_roles[HubREDCapUsers::HUB_ROLE_USER];
            if($this->harmonistadminY){
                #Admin
                $roleId = $user_roles[HubREDCapUsers::HUB_ROLE_ADMIN];
            }
            #USER ID
            ...
            $this->module->query("INSERT INTO redcap_user_rights (" . $fields_rights . ")
                            VALUES (?,?,?,?,?,?,?,?,?,?)",
                                 [$this->getPidsArray()['DATADOWNLOADUSERS'], $this->redcapName, $roleId, 1, 1, 1, 1, 1, 1, $data_entry]);
    ...
    private function isUserExpired(): bool
    {
        $sql = "SELECT * 
                ...
                WHERE username = ? 
                AND user_expiration IS NOT NULL AND user_suspended_time IS NOT NULL";

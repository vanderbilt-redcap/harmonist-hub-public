    ...
    if(!empty($sop)){
        if($sop['follow_activity'] != ''){
            ...
            foreach ($array_userid as $user_id){
                ...
                $name = \Vanderbilt\HarmonistHubPublicExternalModule\getPeopleName($pidsArray['PEOPLE'], $comment['response_person'],"");
                ...
                if(!empty($comment['revised_file'])){
                    $gd_files .= "<li>".\Vanderbilt\HarmonistHubPublicExternalModule\getFileLink($this, $pidsArray['PROJECTS'], $comment['revised_file'],'',0,$secret_key,$secret_iv,$people['record_id'],"")."</li>";
                }
                else{
                ...
                \Vanderbilt\HarmonistHubPublicExternalModule\sendEmail(strtolower($people['email']), $settings['accesslink_sender_email'], $settings['accesslink_sender_name'], $subject, $message,$record,"New data request feedback posted",$project_id);
            }
        }
    }
}

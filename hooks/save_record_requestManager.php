...
if($instrument == 'request'){
    ...
    \REDCap::logEvent("Save Request Data in EM", "record: ".$record, null, $record, $event_id, $project_id);
    ...
    if (empty($concept)) {
        ...
        // Replace any kind of line break with a single space
        $conceptTitle = preg_replace("/\R+/", " ", $request['request_title']);

        $arrayConcepts = array(array('record_id' => $concept_id));
        $arrayConcepts[0]['lastupdate_d'] = $last_update;
        $arrayConcepts[0]['request_id'] = $record;
        $arrayConcepts[0]['active_y'] = "Y";
        $arrayConcepts[0]['concept_id'] = $request['mr_assigned'];
        $arrayConcepts[0]['concept_title'] = $conceptTitle;
        $arrayConcepts[0]['lead_region'] = $request['contact_region'];
        ...
        \REDCap::logEvent("Save Request in Concept, Tracking Number Data in EM", "record: ".$concept_id, null, $concept_id, $event_id, $pidsArray['HARMONIST']);
    ...
    \REDCap::logEvent("Save Request, Admin Review Data in EM", "record: ".$record, null, $record, $event_id, $project_id);
}
...
if (empty($request['workflowcomplete_d'])) {
    if (
        ...
        $json = json_encode($arrayRM);
        $results = \Records::saveData($pidsArray['RMANAGER'], 'json', $json,'overwrite', 'YMD', 'flat', '', true, true, true, false, true, array(), true, false);
        \REDCap::logEvent("Save Request, Workflow Complete Data in EM", "record: ".$record, null, $record, $event_id, $project_id);
    }
}

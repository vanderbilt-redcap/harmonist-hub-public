...
if(($comment[$instrument.'_complete'] == '2' || $vanderbilt_emailTrigger->getEmailTriggerRequested()) && $instrument == 'comments_and_votes'){
    ...
    $regions = arrayKeyExistsReturnValue(\REDCap::getData($pidsArray['REGIONS'], 'json-array', array('record_id' => $comment['response_region'])),[0]);
    if(!empty($regions)){
    ...
    if(!empty($request)){
        ...
        foreach ($request['responding_region'] as $instanceId => $resp_region){
            if($resp_region == $comment['response_region']){
                ...
                \REDCap::logEvent("Save Request Comment, Request Region Data in EM", "record: ".$comment['request_id'], null, $comment['request_id'], $event_id, $pidsArray['RMANAGER']);
                break;
            }
        ...
        if($all_votes_completed && !empty($comment['request_id'])){
            if($request['detected_complete'][1] != "1") {
                ...
                $results = \Records::saveData($pidsArray['RMANAGER'], 'array', $arrayRM,'overwrite', 'YMD', 'flat', '', true, true, true, false, true, [], true, false);
        ...
        if($comment['author_revision_y'] == '1' && $comment['revision_counter'] == ''){
            ...
            $results = \Records::saveData($project_id, 'json', $jsonComment,'overwrite', 'YMD', 'flat', '', true, true, true, false, true, [], true, false);
            \REDCap::logEvent("Save Request Comment, Request Author Revision Data in EM", "record: ".$record, null, $record, $event_id, $project_id);
            ...
            $results = \Records::saveData($pidsArray['RMANAGER'], 'json', $jsonRM,'overwrite', 'YMD', 'flat', '', true, true, true, false, true, [], true, false);
            \REDCap::logEvent("Save Request Comment, Request Revision Total Data in EM", "record: ".$comment['request_id'], null, $comment['request_id'], $event_id, $pidsArray['RMANAGER']);
        ...
        $results = \Records::saveData($project_id, 'array', $arrayCV,'overwrite', 'YMD', 'flat', '', true, true, true, false, true, [], true, false);
        \REDCap::logEvent("Save Request Comment, Email Sent Data in EM", "record: ".$record, null, $record, $event_id, $project_id);
        ...
        if($request['follow_activity'] != ''){
            ...
            foreach ($array_userid as $user_id){
                ...
                $message = '<h2>Feedback Posted on '.$request_type_label[$request['request_type']].' Request  #'.$request['request_id'].'</h2>
                            ...
                            ';
                sendEmail(strtolower($people['email']), $settings['accesslink_sender_email'], $settings['accesslink_sender_name'], $subject, $message,$record,"New request feedback posted", $project_id);
            }
        }
    }
}
